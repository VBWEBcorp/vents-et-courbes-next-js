...
const CoursGrid: React.FC<CoursGridProps> = ({ courses, activeFilter = 'all', onFilterChange }) => {
  ...
  // Cours avec cuisson comprise
  const coursWithCuisson = [
    'tournage-annuel',
    'tournage-trimestriel',
    'modelage-annuel',
    'modelage-trimestriel',
  ...
  const filteredCours = courses.filter(course => {
    ...
    if (activeFilter === 'cuisson') return coursWithCuisson.includes(course.reservationSlug);
    return true;
  });
      ...
      <div className="max-w-7xl mx-auto" ref={gridRef}>
        ...
        <div className="text-center mb-6 fade-in-delay">
          <p className="text-gray-600">
            {filteredCours.length} cours{filteredCours.length > 1 ? '' : ''} 
            {activeFilter !== 'all' && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-6 lg:gap-8 cours-grid">
          {filteredCours.map((course, index) => (
            <Link
              to={`/reservation/${course.reservationSlug}`}
              key={index}
            ...
            >
              ...
              <div className="h-48 lg:h-56 overflow-hidden image-zoom relative">
                ...
                {coursWithCuisson.includes(course.reservationSlug) && (
              ...
              <div className="p-6">
                ...
                <div className="mb-4">
                  <div className="flex items-baseline gap-2 mb-2">
                    ...
                    {course.additional && (
                      <span className="text-green-400 text-lg">{course.additional}</span>
                    )}
                  </div>
                  
                  <div className="text-gray-600 text-sm leading-relaxed">
                    {course.additionalText.split('\n').map((line: string, i: number) => (
                      <div key={i}>{line}</div>
                    ))}
                  </div>
                </div>
